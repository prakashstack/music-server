...
export const musicService = {
  async getTrending(): Promise<any[]> {
    ...
    try {
      ...
      cacheService.set(cacheKey, finalSongs, 1800);
      return finalSongs;
    } catch (err) {
  ...
  async search(query: string, type = 'all', page = 1): Promise<any> {
  ...
  async getSong(id: string, refresh = false): Promise<any> {
    ...
    try {
      ...
      cacheService.set(cacheKey, song, 3600);
      return song;
    } catch {
  ...
  async getGenreSongs(genre: string): Promise<any[]> {
  ...
  async getNewReleases(): Promise<any[]> {
    ...
    try {
      ...
      const songs = result.data?.results || [];
      const finalSongs = songs.length > 0 ? songs : FALLBACK_SONGS;
      cacheService.set(cacheKey, finalSongs, 3600);
      return finalSongs;
    } catch {
      return FALLBACK_SONGS;
    }
